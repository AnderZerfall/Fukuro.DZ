...
document.addEventListener("DOMContentLoaded", () => {
...

  {
    ...
    function FindCursorPosition(event) {
      ...
      let cursorX =
        ((event.clientX - blob.offsetWidth) / containerSize.left) * 100 - 60;
      let cursorY =
        ((event.clientY - blob.offsetHeight) / containerSize.top) * 100 - 100;
    ...
    function ClickOnEye(event) {
      ...
      blob.style.transform =
        "translate(" + cursorX + "%," + cursorY + "%)" + " scaleY(0.2)";
      setTimeout(function () {
        blob.style.transform =
          "translate(" + cursorX + "%," + cursorY + "%)" + " scaleY(1)";
      }, 100);
    }
...

  {
    ...
    function ChangeSlides() {
      ...
      slides[index].style.transform =
        "translateX(" + (slidesNumber - index) * 110 + "%)";
      ...
      for (let i = 1; i <= slidesNumber; i++) {
        if (index + i > slidesNumber) {
          ...
          slides[index + i].style.transform =
...

  {
    ...
    function AnimateSkills(startPoint, endPoint) {
      let skillsAnimation = [{ width: startPoint }, { width: endPoint }];
...

  {
    ...
    function RevealSection() {
      sections.forEach((section) => {
        ...
        if (
          mainScroll + safezone >= sectionTop && // if the top border is under the scroll border
          mainScroll + safezone <= sectionBottom
        ) {
          // while the bottom border is over it
